...
#     mapy = np.zeros(mapsize, dtype=np.float32)
    ...
    for x in range(quads_per_row+1):
        for y in range(quads_per_col+1):
            pt_dst_all.append([y*pixels_per_row, x*pixels_per_col])
...
#     for ar in g_out:
#         tmp_g_out = 
#     tmp_g = np.array([arx[:,0] for arx in g_out])
...
#     print distorted_grid
    cv2.imwrite('./mesh_src.png', src)
    cv2.imwrite('./mesh_dst.png', dst)
    return dst
